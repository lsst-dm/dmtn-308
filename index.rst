...
User access to the PPDB will be provided by an `IVOA TAP service <https://www.ivoa.net/documents/TAP/>`_ through the Rubin Science Platform (RSP), allowing users to query the database using the `Astronomical Data Query Language <https://www.ivoa.net/documents/ADQL/>`_ (ADQL).
...
- The PostgreSQL database engine running in a single server mode has a number of limitations which prevent it from scaling to the required data volumes.
...
- Overall, given these constraints and limitations, a single PostgreSQL instance cannot scale to the data volume requirements of the PPDB under any retention scenario being considered.
...
- Citus should be able to handle the data volume requirements of the PPDB under any retention scenario being considered.
...
- The above is not a comprehensive list of all limiting features, but it is clear that AlloyDB would not be suitable for the data volumes required for the PPDB.

BigQuery
~~~~~~~~

- BigQuery is a MPP database engine designed for unlimited scalability.
   - Storage and compute are decoupled, with data stored in Google's Colossue file system.
   - Storage essentially has unlimited scalability without impacting query performance.
...
- Overall, BigQuery can handle petabytes of data, so it should be able to meet the data volume requirements of the PPDB.
...
- Citus can achieve high query performance on large datasets, as it is designed to scale out horizontally across multiple nodes.
   ...
   - Citus employs adaptive query optimization, optimizing for minimal data movement and efficient execution.
      - Queries are rewritten to lverage parallelism and avoid unnecessary computation.
      - Joins are optimized by pushing computations to worker nodes to reduce cross-shard communication.
- Columnar storage is supported for analytical workloads, alloing for efficient scanning of required coumns, which can improve performance for large-scale queries, e.g., full table scans.
...
- Qserv is highly performant for large datasets, as it is designed to scale out horizontally across multiple nodes.
   - Tables are spatially sharded, and low latency can be achieved for restricted spatial queriese, e.g., cone searches, typically.
...
- With certain reservations, Qserv should be able to achieve adequate query performance for the data volumes expected for the PPDB.
...
- Neither storage nor compute scalability is achieveable at the level required for the PPDB.
...
- Elasticity can be achieved by adding nodes to the cluster or removing them, but these operations requires table resharding and balancing, which can be complex and time-consuming.
   - Though in theory Citus can achieve elasticity, in practice it may be difficult to achieve in a production environment.
...
- Compute resources, or "slots" in BigQuery terminology, are provisioned dynamically for each query, allowing for unlimited scalability to meet demand.
- This platform also has excellent elasticity, as resources can be provisioned and deprovisioned on-the-fly, with no need for manual intervention.
...
- PostgreSQL has low operating costs to the project for on-premises deployments.
...
- However, Citus would incur much higher hardware costs than a single-node deployment, as it would require multiple nodes to be provisioned with sufficient memory, CPU, and storage to meet the expected data volume and query load.
...
- Overall, without favorable pricing agreements, AlloyDB would likely be an expensive platform for the PPDB, incurring high operating costs, which would grow over time with more data and queries.
...
- Though the default BigQuery pricing structure would likely result in very high operating costs, it is possible that significant discounts could be negotiated, given the scientific and governmental nature of the project.
...
- Management of BigQuery would still rely to some extend on expertise of Rubin personnel, who do not have much experience with the platform.
...
- However, Citus would require a significant amount of development effort to create or adapt Kubernetes operators or Helm charts, backup and recovery solutions, and other tools to manage the distributed database.

Qserv
~~~~~

- Qserv would require very high developer effort, initially on the order of 1 FTE or more, because it lacks many required features, including, but not limited to tooling for data ingestion.
...
- However, no testing has been done with this platform, and the distribution of data across worker nodes could complicate the process.
   - Additional testing would be required to ensure that the replication tools can handle the expected data volume of the PPDB.
- Bottlenecks on the coordinator node could be a concern, as it would be responsible for managing ingestion while also servicing user queries.
...
- A significant amount of development effort would be required to implement this functionality.
   ...
   - For instance, data in Parquet format dumped from the APDB might be loaded into Google Cloud Storage, triggering an ETL process that loaded the data, rather than using the streaming operations from the current implementation.
- Not having these tools available is a significant roadblock for using BigQuery as a backend.
...
- Though more limited than PostgreSQL, Citus's community and ecosystem should not be considered a significant limiting factor.

Qserv
~~~~~

- As an in-house platform, Qserv has an extremely limited ecosystem and community.
...
- Though more limited than PostgreSQL and (likely) Citus, this is probably not a significant limiting factor in terms of platform selection, as the existing resources seem adequate.
...
It should be clear that there is no clear winner among the database platforms considered, though given the requirements and constraints, several of them can be eliminated entirely as realistic options.
...
- **A single PostgreSQL server is not a suitable platform for the PPDB and can be eliminated as a viable option.**

Citus
-----

- Citus brings with it all of the positive features of PostgreSQL, as it is an extension of the platform.
- The platform is designed to scale out horizontally, and it should be able to handle the data volume and query performance requirements of the PPDB.
...
Given the information present above, we provide the following ordered recommendation of database platforms for the PPDB:
...
Data ingestion tools would also need to be written, but this is a relatively straightforward process, as BigQuery has a well-documented API and many libraries available for interacting with the service.

Finally, strategic considerations related to the broader astronomical community and the hosting of massive datasets in the cloud should be considered. The community is moving towards cloud-based solutions, and the PPDB could be a flagship project for Rubin in this area.

2. Citus
--------

Citus has an extremely attractive feature set, as it is an extension of PostgreSQL and is designed to scale out horizontally across multiple nodes.
Its documentation claims that petabyte scalability is achievable given the right hardware and configuration.
Existing tools that have already been developed for PostgreSQL should work with Citus, and the platform should be able to handle the data volume and query performance requirements of the PPDB.
...
A significant amount of administrative and developer effort would required to develop configuration and monitoring tools, as well as Kubernetes operators or Helm charts, backup and recovery solutions, and other tools to manage the distributed database.
It is not clear that there is sufficient manpwer available for such an effort, and the cost of purchasing the necessary hardware would likely be high.
